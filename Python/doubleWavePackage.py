...
fun = np.exp(-1/2 * (x-x0)**2/sigma**2) * np.exp(-1j*k0*x) + np.exp(-1/2 * (x-(1-x0))**2/sigma**2) * np.exp(-1j*(-k0)*x)
...
anim.save("doubleWavePackage.webm", writer="ffmpeg", fps = 30, codec="vp9", dpi=200)
